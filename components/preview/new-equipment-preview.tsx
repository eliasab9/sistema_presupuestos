          ...
          <div style={{ width: scaledW, height: scaledH, margin: '0 auto', overflow: 'hidden' }}>
            ...
            >
              ...
              <header style={{
                ...
                borderBottom: `3px solid ${primaryColor}`, paddingBottom: '10px', marginBottom: '15px',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  />
                  <div>
                    <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: primaryColor, margin: 0 }}>
                      {company.name}
                    </h1>
                    <p style={{ fontSize: '10px', color: '#666', margin: 0 }}>{company.subtitle}</p>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <h2 style={{ fontSize: '14px', fontWeight: 'bold', margin: 0, marginBottom: '4px' }}>
                    PRESUPUESTO N° {meta.number}
                  </h2>
                  <p style={{ fontSize: '9px', color: '#666', margin: '2px 0' }}>Fecha: {meta.date}</p>
                  <p style={{ fontSize: '9px', color: '#666', margin: '2px 0' }}>Válido hasta: {meta.validUntil}</p>
                  <p style={{ fontSize: '9px', color: '#666', margin: '2px 0' }}>TC: ${meta.exchangeRate.toLocaleString('es-AR')} / U$S</p>
                </div>
              </header>
              ...
              <section style={{ marginBottom: '15px' }}>
                <h3 style={{
                  fontSize: '10px', fontWeight: 'bold', color: primaryColor, textTransform: 'uppercase',
                  marginBottom: '8px', borderBottom: `1px solid ${primaryColor}`, paddingBottom: '3px',
                }}>
                  Datos del Cliente
                </h3>
                <div style={{
                  display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 20px',
                  fontSize: '10px', backgroundColor: '#f9f9f9', padding: '8px',
                }}>
                  <div>
              ...
              <section style={{ marginBottom: '15px' }}>
                ...
                {items.length === 0 ? (
                  <p style={{ fontSize: '10px', color: '#999', fontStyle: 'italic' }}>No hay equipos agregados</p>
                ) : (
                        ...
                        <tr key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                          <td style={{ padding: '6px 0' }}>
                            ...
                            {item.type === 'otro' && item.customSpecs && (
                              <div style={{ fontSize: '8px', color: '#666', marginTop: '2px' }}>{item.customSpecs}</div>
                            )}
                            {item.notes && (
                              <div style={{ fontSize: '8px', color: '#666', marginTop: '2px' }}>Nota: {item.notes}</div>
                            )}
                          </td>
              ...
              <section style={{ marginBottom: '15px' }}>
                ...
                <ul style={{ fontSize: '9px', color: '#555', margin: 0, paddingLeft: '15px' }}>
                  {meta.commercialValidity && <li style={{ marginBottom: '3px' }}>Validez: {meta.commercialValidity}.</li>}
                  {meta.paymentTerms && <li style={{ marginBottom: '3px' }}>Forma de pago: {meta.paymentTerms}.</li>}
              ...
              <footer style={{
                ...
                borderTop: '1px solid #ccc', paddingTop: '10px', textAlign: 'center',
              }}>
                <p style={{ fontSize: '10px', fontWeight: 'bold', color: primaryColor, margin: 0 }}>
